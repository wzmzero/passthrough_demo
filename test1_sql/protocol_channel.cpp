    ...
    : name_(name)
{
    ...
    node1_->setReceiveCallback([this](const uint8_t* data, size_t len) {
        if (!node1_to_node2_buffer_.push(data, len)) {
            logHandler("Node1->Node2 buffer overflow, dropped " + std::to_string(len) + " bytes");
        }
    });
    
    node2_->setReceiveCallback([this](const uint8_t* data, size_t len) {
        if (!node2_to_node1_buffer_.push(data, len)) {
            logHandler("Node2->Node1 buffer overflow, dropped " + std::to_string(len) + " bytes");
        }
    });
}
...
void ProtocolChannel::node1ToNode2Worker() {
    logHandler("Node1->Node2 worker started");
    while (running_) {
        auto data = node1_to_node2_buffer_.pop(4096);
        if (data) {
            node2_->send(data->data(), data->size());
        }
    }
    logHandler("Node1->Node2 worker exited");
}

void ProtocolChannel::node2ToNode1Worker() {
    logHandler("Node2->Node1 worker started");
    while (running_) {
        auto data = node2_to_node1_buffer_.pop(4096);
        if (data) {
            node1_->send(data->data(), data->size());
        }
    }
    logHandler("Node2->Node1 worker exited");
}
